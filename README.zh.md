...
**Obsidian 中国象棋插件** 是一款为 Obsidian 笔记软件量身打造的中国象棋渲染引擎，支持以 FEN 和 PGN 格式展示棋局、推演走棋、管理分支变着。支持可自定义名称的代码块（`xiangqi`/`tree` 为分支图模式，`xq` 为 FEN 编辑器）、响应式布局、内置 [皮卡鱼](https://github.com/official-pikafish/Pikafish) (WASM) 引擎分析、语音朗读等功能。
...
- **引擎分析**：内置皮卡鱼 WASM 引擎，支持单步分析、自动分析和批量分析
...
一种代码块，可自定义多个名称——`xiangqi` 和 `tree` 是同一代码块的默认别名，`xq` 是 FEN 编辑器代码块。
...
`tree`：分支图，以树状图展示棋局变着，菜单式工具栏（编辑/节点/分析三个菜单）
...
在 **设置 > Chess > 代码块名称** 中自定义代码块别名（需重启生效）：

- **分支图模式**（xiangqi/tree）：默认 `xiangqi, tree`，可添加自定义别名

### 引擎分析

- **引擎深度**：皮卡鱼搜索深度滑块（1–30，默认 18）
...
5. 点击有多个子节点的折叠按钮（三角形）可以折叠/展开其变着分支
6. 引擎分析功能：
   - 分析菜单：设置深度、单次分析、自动分析（切换）、批量分析
   - 分析按钮：默认 → 激活（自动分析开启时高亮）→ 忙碌（引擎运行时闪烁）
   - 自动分析跳过已分析到当前深度的节点
   - 绿色箭头 = 最佳走法，黄色箭头 = 思考走法
   - 左侧评估条显示当前局面评估
