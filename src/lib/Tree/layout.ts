...
function insertBySide(
  arr: ChessNode[],
  nodes: ChessNode | ChessNode[],
  target: ChessNode,
  nodemap: NodeMap,
): void {
  let nonMainAncestor;
  ...
  // 如果是黑方节点，先反转子节点顺序
  const orderedNodes = insertNodes[0].side === 'black' ? [...insertNodes].reverse() : insertNodes;
...
export function calculateTreeLayout(nodeMap: NodeMap): ChessNode[] {
  ...
  return Array.from(nodeMap.values()).filter(n => n.x !== undefined && n.y !== undefined);
}
